...
const Home = () => {
    ...
    const handleSearch = useCallback(
        debounce(async (text) => {
            try {
                const items = await fetchFilteredMenuItems(text, selectedCategories);
                setMenuItems(items);
            } catch (error) {
    ...
    const renderItem = ({ item }) => (
        <View style={styles.menuItem}>
            ...
            <Image
                source={{
                    uri: `https://github.com/Meta-Mobile-Developer-PC/Working-With-Data-API/blob/main/images/${encodeURIComponent(
                        item.image
                    )}?raw=true`,
                }}
                style={styles.image}
